...
class hcp_dataset(Dataset):
    def __init__(
        ...
        self.images = images
        self.images_transform = transforms.Compose([img_norm, transforms.Resize((112, 112)), channel_first])
    ...
    def __getitem__(self, index):
        ...
        return {"fmri": self.transform(self.data[index]), "image": images_transform(img)}
...
def create_Kamitani_dataset(
    path="../data/Kamitani/npz",
    roi="VC",
    patch_size=16,
    ...
    for sub in subjects:
        ...
        if test_category is not None:
            train_img_, train_fmri_, test_img_, test_fmri_, train_lb, test_lb = reorganize_train_test(
                train_img[-1],
                train_fmri[-1],
                test_img[-1],
                test_fmri[-1],
...
def reorganize_train_test(
    train_img, train_fmri, test_img, test_fmri, train_lb, test_lb, test_category, train_index_lookup
):
    test_img_ = []
    test_fmri_ = []
    test_lb_ = []
...
class Kamitani_dataset(Dataset):
    def __init__(
        self, fmri, image, img_label, fmri_transform=identity, image_transform=identity, num_voxels=0, num_per_sub=50
    ):
    ...
    def __getitem__(self, index):
        ...
        if self.return_image_class_info:
            ...
            return {
                ...
                "image_class_name": img_class_name,
...
def create_BOLD5000_dataset(
    ...
    imgs_dict = np.load(
        os.path.join(img_path, "Scene_Stimuli/Presented_Stimuli/img_dict.npy"), allow_pickle=True
    ).item()
    ...
    if isinstance(image_transform, list):
        return (
            BOLD5000_dataset(fmri_train_major, img_train_major, fmri_transform, image_transform[0], num_voxels),
            BOLD5000_dataset(fmri_test_major, img_test_major, torch.FloatTensor, image_transform[1], num_voxels),
        )
    else:
        return (
            BOLD5000_dataset(fmri_train_major, img_train_major, fmri_transform, image_transform, num_voxels),

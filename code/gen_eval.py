...
def wandb_init(config):
    wandb.init(project="mind-vis", group="eval", anonymous="allow", config=config, reinit=True)
...
def get_eval_metric(samples, avg=True):
    ...
    for s in samples_to_run:
        ...
        res = get_similarity_metric(
            pred_images, gt_images, "class", None, n_way=50, num_trials=1000, top_k=1, device="cuda"
        )
        res_part.append(np.mean(res))
    res_list.append(np.mean(res_part))
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(
        config.root_path, "results", "eval", "%s" % (datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S"))
    )

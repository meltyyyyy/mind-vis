...
class fLDM:
    ...
    def finetune(self, trainers, dataset, test_dataset, bs1, lr1, output_path, config=None):
        ...
        torch.save(
            {"model_state_dict": self.model.state_dict(), "config": config, "state": torch.random.get_rng_state()},
    ...
    def generate(self, fmri_embedding, num_samples, ddim_steps, HW=None, limit=None, state=None):
        ...
        with model.ema_scope():
            ...
            for count, item in enumerate(fmri_embedding):
                ...
                samples_ddim, _ = sampler.sample(
                    S=ddim_steps, conditioning=c, batch_size=num_samples, shape=shape, verbose=False
                )
        ...
        return grid, (255.0 * torch.stack(all_samples, 0).cpu().numpy()).astype(np.uint8)

...
from dc_ldm.models.autoencoder import AutoencoderKL, IdentityFirstStage, VQModelInterface
from dc_ldm.models.diffusion.ddim import DDIMSampler
from dc_ldm.models.diffusion.plms import PLMSSampler
from dc_ldm.modules.diffusionmodules.util import extract_into_tensor, make_beta_schedule, noise_like
from dc_ldm.modules.distributions.distributions import DiagonalGaussianDistribution, normal_kl
...
class DDPM(pl.LightningModule):
    ...
    def generate(self, data, num_samples, ddim_steps=300, HW=None, limit=None, state=None):
        ...
        if HW is None:
            ...
            shape = (self.p_channels, HW[0] // 2 ** (num_resolutions - 1), HW[1] // 2 ** (num_resolutions - 1))
        ...
        with model.ema_scope():
            for count, item in enumerate(zip(data["fmri"], data["image"])):
                ...
                samples_ddim, _ = sampler.sample(
                    S=ddim_steps, conditioning=c, batch_size=num_samples, shape=shape, verbose=False, generator=None
                )
                ...
                all_samples.append(
                    torch.cat([gt_image.detach().cpu(), x_samples_ddim.detach().cpu()], dim=0)
        ...
        return grid, (255.0 * torch.stack(all_samples, 0).cpu().numpy()).astype(np.uint8), state
            ...
            for sp_idx, imgs in enumerate(all_samples):
                for copy_idx, img in enumerate(imgs[1:]):
                    ...
                    Image.fromarray(img).save(
                        os.path.join(
                            self.output_path, "val", f"{self.validation_count}_{suffix}", f"test{sp_idx}-{copy_idx}.png"
                        )
                    )
    ...
    def full_validation(self, batch, state=None):
        ...
        if metric[-1] > self.best_val:
            ...
            torch.save(
                {"model_state_dict": self.state_dict(), "config": self.main_config, "state": state},
    ...
    def get_eval_metric(self, samples, avg=True):
        ...
        for s in samples_to_run:
            ...
            res = get_similarity_metric(
                pred_images, gt_images, "class", None, n_way=50, num_trials=50, top_k=1, device="cuda"
            )
            res_part.append(np.mean(res))
        res_list.append(np.mean(res_part))
...
class LatentDiffusion(DDPM):
    ...
    def configure_optimizers(self):
        ...
        if self.train_cond_stage_only:
            ...
            cond_parms = [
                p for n, p in self.named_parameters() if "attn2" in n or "time_embed_condtion" in n or "norm2" in n
            ]

...
class wandb_logger:
    def __init__(self, config):
        wandb.init(project="mind-vis", group="stepA_sc-mbm_tune", anonymous="allow", config=config, reinit=True)
...
def main(config):
    ...
    output_path = os.path.join(
        config.root_path, "results", "fmri_finetune", "%s" % (datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S"))
    )
    ...
    for ep in range(config.num_epoch):
        ...
        if (ep % 2 == 0 or ep + 1 == config.num_epoch) and ep != 0 and config.local_rank == 0:
            # save models
            save_model(
                config_pretrain, ep, model_without_ddp, optimizer, loss_scaler, os.path.join(output_path, "checkpoints")
            )
            # plot figures
...
def plot_recon_figures(
    ...
    for ax in axs:
        ...
        sample_with_mask = (
            model_without_ddp.patchify(sample).to("cpu").numpy().reshape(-1, model_without_ddp.patch_size)
        )
